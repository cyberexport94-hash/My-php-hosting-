...
<div class="wrapper">
    ...
    <section class="panel">
        <h2>&gt; WRITE_CODE // DEPLOY DIRECTLY</h2>
        <form action="create.php" method="POST" class="code-form">
            <div class="code-form-row">
                <input type="text" name="filename" placeholder="file name (bina extension ke)" required class="code-filename">
                <select name="type" class="code-type">
                    <?php foreach (CODE_TYPES as $ext => $label): ?>
                        <option value="<?= $ext ?>"><?= $label ?> (.<?= $ext ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            <textarea name="code" rows="10" placeholder="&lt;!-- yahan apna HTML / CSS / JS code likhein --&gt;" required class="code-textarea"></textarea>
            <button type="submit" class="btn-deploy">⚡ WRITE &amp; DEPLOY</button>
        </form>
        <p class="hint">Code seedha likh kar submit karein — automatically file bankar deploy ho jayega aur live link mil jayega. Max size: <?= human_size(MAX_CODE_SIZE) ?></p>
    </section>
